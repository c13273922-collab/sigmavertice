-- ============================================================================
--  SIGMA VÉRTICE · SCRIPT COMPLETO ÚNICO 1-CLIQUE
--  Migration + Seed + Vínculo DG · Tudo do Zero, sem Erros
--
--  ANTES DE RODAR ISTO: GARANTA QUE RODOU O PASSO_1_RESET_TOTAL_APAGAR_TUDO.sql
--  (Se não tiver certeza, rode ele mesmo assim — o reset é idempotente / seguro)
-- ============================================================================
...
-- ======================================================
--  PARTE 2 · TODOS OS ENUMS (DO blocks exception)
-- ======================================================
do $$ begin
  create type public.role_sistema as enum (
    'diretor_geral','diretor_central','coordenador','funcionario'
  );
exception when duplicate_object or unique_violation then null; end $$;

do $$ begin
  create type public.status_generico as enum (
...
do $$ begin
  create type public.nivel_alerta as enum ('baixo','medio','alto','critico');
exception when duplicate_object or unique_violation then null; end $$;

do $$ begin
  create type public.categoria_advertencia as enum (
...
do $$ begin
  create type public.tipo_auditoria as enum (
...
-- ======================================================
--  PARTE 3 · TABELAS (ORDEM 100% CORRETA, SEM FK ANTECIPADA)
-- ======================================================
...
-- 3. PERMISSÕES (só depende de enum criado acima)
create table public.permissoes (
  id uuid primary key default uuid_generate_v4(),
  chave varchar(120) not null unique,
  modulo varchar(80) not null,
  descricao text,
  nivel_requerido public.role_sistema not null default 'funcionario',
  created_at timestamptz not null default now()
);

-- 4. UNIDADES (apenas FK municipios — tabela 1)
create table public.unidades (
  ...
  telefone varchar(40),
  email varchar(160),
  responsavel_id uuid,                     -- FK adicionada no ALTER TABLE POSTERIORMENTE
...
-- 5. FUNCIONÁRIOS (FK p/ auth.users, municipios 1, unidades 4, cargos 2 — todos existem)
create table public.funcionarios (
  ...
  role_sistema public.role_sistema not null default 'funcionario',
  data_admissao date,
  data_desligamento date,
  motivo_desligamento text,
  periodo_impedimento date,
  foto_url text,
  status_registro public.status_generico not null default 'ativo',
...
-- 6. ROLE_PERMISSOES (FK permissoes 3)
create table public.role_permissoes (
  id uuid primary key default uuid_generate_v4(),
  role_sistema public.role_sistema not null,
  permissao_id uuid not null references public.permissoes(id) on delete cascade,
  unique(role_sistema, permissao_id)
);

-- 7. FUNCIONARIO_PERMISSOES (FK funcionarios 5, permissoes 3, funcionarios 5 concedido_por)
...
create table public.sessoes (
  ...
  status public.status_generico not null default 'online',
  revogada boolean default false,
  motivo_revogacao text,
  created_at timestamptz not null default now()
);

-- 9. PROCESSOS (FK municipios, unidades, funcionarios)
create table public.processos (
  ...
  municipio_id uuid references public.municipios(id) on delete set null,
  unidade_id uuid references public.unidades(id) on delete set null,
  prioridade public.nivel_alerta default 'medio',
  ...
  prazo date,
  status public.status_generico not null default 'novo',
...
-- 10. CASOS (FK municipios, unidades, funcionarios, processos)
create table public.casos (
  ...
  responsavel_id uuid references public.funcionarios(id) on delete set null,
  prazo date,
  status public.status_generico not null default 'disponivel',
  anexos jsonb default '[]'::jsonb,
  historico jsonb default '[]'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
...
-- 11. VIAGENS (FK municipios, funcionarios)
create table public.viagens (
  ...
  observacoes text,
  criado_por uuid not null references public.funcionarios(id),
  status public.status_generico not null default 'aguardando_aprovacao',
  anexos jsonb default '[]'::jsonb,
  historico jsonb default '[]'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
...
-- 12. SOLICITACOES (FK funcionarios)
create table public.solicitacoes (
  ...
  status public.status_generico not null default 'solicitacao_pendente',
...
create table public.aprovacoes (
  ...
  decisao public.status_generico check (decisao in ('aprovado','recusado','solicitar_correcao')),
  observacao text,
  data_decisao timestamptz,
  created_at timestamptz not null default now()
);

-- 14. CONVERSAS (FK funcionarios)
...
-- 15. MENSAGENS (FK conversas, funcionarios)
...
-- 16. ARQUIVOS (FK mensagens, processos, casos, viagens, funcionarios)
create table public.arquivos (
  ...
  status public.status_generico default 'ativo',
  created_at timestamptz not null default now()
);

-- 17. DOCUMENTOS (FK arquivos, processos, casos, viagens, funcionarios, municipios)
...
-- 18. TRANSACOES (FK municipios, funcionarios, documentos, arquivos)
...
create table public.noticias (
  ...
  categoria public.categoria_noticia default 'geral',
  local varchar(160),
  municipio_id uuid references public.municipios(id) on delete set null,
  uf char(2),
...
-- 20. FALTAS (FK funcionarios, documentos)
create table public.faltas (
  ...
  analisado_por uuid references public.funcionarios(id) on delete set null,
  analisado_em timestamptz,
  status public.status_generico not null default 'pendente',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

-- 21. ADVERTENCIAS (FK funcionarios, documentos)
create table public.advertencias (
  id uuid primary key default uuid_generate_v4(),
  protocolo varchar(50) unique not null,
  funcionario_id uuid not null references public.funcionarios(id) on delete cascade,
  categoria public.categoria_advertencia not null,
...
-- 22. DESLIGAMENTOS (FK funcionarios, documentos)
create table public.desligamentos (
  ...
  documento_id uuid references public.documentos(id) on delete set null,
  aplicado_por uuid not null references public.funcionarios(id),
  status public.status_generico not null default 'desligado',
  observacoes text,
  created_at timestamptz not null default now()
);

-- 23. COMUNICADOS (FK funcionarios)  · publicado_por = NULLABLE para evitar erro circular
create table public.comunicados (
  ...
  resumo text,
  publicado_por uuid references public.funcionarios(id),        -- SEM NOT NULL (permite NULL como fallback)
  destinatarios_roles public.role_sistema[] default null,
...
-- 24. ALERTAS (FK municipios, unidades, funcionarios)
create table public.alertas (
  id uuid primary key default uuid_generate_v4(),
  titulo varchar(240) not null,
  descricao text,
  nivel public.nivel_alerta not null default 'medio',
  categoria varchar(60),
  modulo varchar(60),
  referencia_id uuid,
  municipio_id uuid references public.municipios(id) on delete set null,
...
-- 25. INDICADORES (FK municipios, unidades)
...
-- 26. BENEFICIOS (FK funcionarios, documentos)
...
-- 27. RELATORIOS (FK arquivos, funcionarios)
...
-- 29. AUDITORIA (FK funcionarios, sessoes)
create table public.auditoria (
  id bigserial primary key,
  tipo public.tipo_auditoria not null,
  modulo varchar(80),
  entidade varchar(80),
  entidade_id uuid,
  acao varchar(160),
...
-- 30. DISTRIBUICOES (FK funcionarios)
...
-- 31. MUNICIPIOS_METRICAS (FK municipios)
create table public.municipios_metricas (
...
-- ======================================================
--  PARTE 3.1 · FKs pendentes adicionadas via ALTER TABLE
-- ======================================================
do $$ begin
...
-- ======================================================
--  PARTE 4 · TRIGGERS (updated_at + auditoria)
-- ======================================================
...
begin
  ...
  if (tg_op = 'INSERT') then
    ...
    v_tipo   := case tg_table_name
      when 'funcionarios'  then 'criacao_funcionario'::public.tipo_auditoria
      when 'processos'     then 'criacao_processo'::public.tipo_auditoria
      when 'casos'         then 'criacao_caso'::public.tipo_auditoria
      when 'viagens'       then 'criacao_viagem'::public.tipo_auditoria
      when 'transacoes'    then 'criacao_transacao'::public.tipo_auditoria
      when 'advertencias'  then 'criacao_advertencia'::public.tipo_auditoria
      when 'desligamentos' then 'criacao_desligamento'::public.tipo_auditoria
      else 'outro'::public.tipo_auditoria
    end;
  elsif (tg_op = 'UPDATE') then
    v_depois := to_jsonb(new);
    v_antes  := to_jsonb(old);
    v_tipo   := case tg_table_name
      when 'funcionarios'  then 'alteracao_funcionario'::public.tipo_auditoria
      when 'processos'     then 'alteracao_processo'::public.tipo_auditoria
      when 'casos'         then 'alteracao_caso'::public.tipo_auditoria
      when 'viagens'       then 'alteracao_viagem'::public.tipo_auditoria
      when 'transacoes'    then 'alteracao_transacao'::public.tipo_auditoria
      when 'advertencias'  then 'alteracao_advertencia'::public.tipo_auditoria
      when 'desligamentos' then 'alteracao_desligamento'::public.tipo_auditoria
      else 'outro'::public.tipo_auditoria
    end;
  else
    return null;
...
create index idx_funcionarios_unidade   on public.funcionarios(unidade_id);
create index idx_funcionarios_cargo     on public.funcionarios(cargo_id);
create index idx_funcionarios_status    on public.funcionarios(status_registro);
create index idx_funcionarios_role      on public.funcionarios(role_sistema);

create index idx_processos_municipio on public.processos(municipio_id);
...
create index idx_casos_processo on public.casos(processo_id);
create index idx_casos_municipio on public.casos(municipio_id);
create index idx_casos_status    on public.casos(status);
create index idx_casos_responsavel on public.casos(responsavel_id);

create index idx_viagens_municipio on public.viagens(municipio_id);
create index idx_viagens_status    on public.viagens(status);
create index idx_viagens_funcionario on public.viagens(funcionario_id);
create index idx_viagens_periodo on public.viagens(data_saida, data_retorno);

create index idx_conversas_a on public.conversas(participante_a);
create index idx_conversas_b on public.conversas(participante_b);
create index idx_mensagens_conv on public.mensagens(conversa_id);
create index idx_mensagens_data on public.mensagens(created_at);

create index idx_auditoria_tipo on public.auditoria(tipo);
create index idx_auditoria_func on public.auditoria(funcionario_id);
create index idx_auditoria_data on public.auditoria(created_at desc);

create index idx_sessoes_func on public.sessoes(funcionario_id);
create index idx_sessoes_status on public.sessoes(status);

create index idx_alertas_nivel on public.alertas(nivel);
create index idx_alertas_func  on public.alertas(destinatario_id);
create index idx_alertas_data  on public.alertas(created_at desc);
...
create index idx_noticias_municipio on public.noticias(municipio_id);

create index idx_advertencias_func on public.advertencias(funcionario_id);
create index idx_faltas_func on public.faltas(funcionario_id);
create index idx_municipios_uf on public.municipios(uf);

-- ======================================================
--  PARTE 6 · RLS + POLÍTICAS
-- ======================================================
...
returns public.role_sistema language sql stable security definer as $$
  select coalesce(
    (select role_sistema from public.funcionarios where id = auth.uid()),
    'funcionario'::public.role_sistema
  );
$$;
...
-- ======================================================
--  PARTE 7 · SEED COMPLETO (cargos, permissoes, municipios AM, unidades, configs)
-- ======================================================
...
-- 7.3 Role Permissões (com CAST para enum)
insert into public.role_permissoes (role_sistema, permissao_id)
select r.rs::public.role_sistema, p.id
from (values
  ('diretor_geral'),('diretor_central'),('coordenador'),('funcionario')
) r(rs)
...
-- 7.5 Unidades
do $$
declare v_manaus uuid;
declare v_tefe   uuid;
declare v_par    uuid;
declare v_tab    uuid;
begin
  select id into v_manaus from public.municipios where codigo_ibge='1302505';
  select id into v_tefe   from public.municipios where codigo_ibge='1303958';
  select id into v_par    from public.municipios where codigo_ibge='1303008';
  select id into v_tab    from public.municipios where codigo_ibge='1303800';

  insert into public.unidades (codigo, nome, tipo, municipio_id, capacidade, ativo)
  values
    ('UNI-HQ-001','Sede Principal Manaus (Diretoria Geral)','Sede',v_manaus,2000,true),
    ('UNI-AM-002','Unidade Manaus - Centro Operacional','Operacional',v_manaus,1500,true),
    ('UNI-AM-003','Unidade Manaus - Inteligência de Dados','Inteligência',v_manaus,500,true)
  on conflict (codigo) do nothing;

  insert into public.unidades (codigo, nome, tipo, municipio_id, capacidade, ativo)
  values ('UNI-TEF-001','Unidade Tefé - Polo Sul do Amazonas','Operacional',v_tefe,600,true)
  on conflict (codigo) do nothing;

  insert into public.unidades (codigo, nome, tipo, municipio_id, capacidade, ativo)
  values ('UNI-PAR-001','Unidade Parintins - Polo Leste','Operacional',v_par,500,true)
  on conflict (codigo) do nothing;

  insert into public.unidades (codigo, nome, tipo, municipio_id, capacidade, ativo)
  values ('UNI-TAB-001','Unidade Tabatinga - Polo Fronteiriço','Fronteiriça',v_tab,450,true)
  on conflict (codigo) do nothing;
end $$;
...
begin
  ...
  select id into v_unid  from public.unidades   where codigo='UNI-HQ-001' limit 1;
  ...
  insert into public.funcionarios (
    ...
    municipio_id, unidade_id, cargo_id, role_sistema, data_admissao, status_registro
  ) values (
...
-- 7.9 Comunicados (publicado_por = 1o funcionario, fallback null)
...
do $$ begin
  raise notice E'\n\n'
               '==============================================================\n'
               '  🎉 SCRIPT COMPLETO 1-CLIQUE EXECUTADO COM SUCESSO!  🎉\n'
               '==============================================================\n\n'
               '✅ 32 TABELAS criadas com sucesso\n'
               '✅ 8 TIPOS ENUM criados\n'
               '✅ Triggers updated_at + auditoria ativos em todas tabelas\n'
               '✅ 60+ ÍNDICES criados (performance)\n'
               '✅ RLS ativo em todas tabelas (25+ policies)\n'
               '✅ Seed carregado:\n'
               '     · 19 Cargos\n'
               '     · 34 Permissões + 4 roles\n'
               '     · 62 Municípios do Amazonas (com IBGE)\n'
               '     · 5 Unidades (HQ Manaus, OP, INT, TEF, PAR, TAB)\n'
               '     · 16 Configurações do sistema (MFA 8h ADV1/2/3=700/800/900 etc)\n'
               '     · Métricas zeradas para todos municípios\n'
               '     · 3 Comunicados iniciais (Bem-vindo, Sigilo, Servidores)\n'
               '     · 5 Notícias exemplo (feed monitoramento Brasil)\n\n'
               '✅ Funcionário DIRETOR GERAL vinculado:\n'
               '     UUID  = 3a9eb272-e1f1-4024-ad7e-365248b3d628\n'
               '     Email = c13273822@gmail.com   (use este no login)\n'
               '     Senha = a que você criou em Authentication → Users\n\n'
               '==============================================================\n'
               '>>>  PRÓXIMOS PASSOS (fora do SQL):\n'
               '     1) Supabase → Settings → API: copiar 3 chaves.\n'
               '     2) Colar em .env.local do projeto (local)\n'
               '     3) Copiar as MESMAS 3 na Vercel → Env Vars.\n'
               '     4) Deploy na Vercel.\n'
               '     5) Painel SuperDomínios → DNS → apontar sigmavertice.org para Vercel.\n'
               '==============================================================\n';
end $$;
