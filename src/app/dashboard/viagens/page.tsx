...
export default function ViagensPage() {
  ...
  function statusInfo(s: string) {
    if (s === 'aprovacao') return { t: 'Aguardando DG', c: 'bg-purple-500/15 text-purple-700' }
    if (s === 'aprovada') return { t: 'Aprovada', c: 'bg-emerald-500/15 text-emerald-700' }
    if (s === 'disponivel') return { t: 'Para assumir', c: 'bg-blue-500/15 text-blue-700' }
    return { t: 'Concluída', c: 'bg-slate-500/15 text-slate-700' }
  }
              ...
              <div key={v.protocolo} className="rounded-xl border border-sigma-azul/10 p-5 hover:shadow-md hover:border-sigma-dourado/30 transition-all animate-fade-in-up" style={{ animationDelay: `${i * 0.05}s` }}>
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold ${b.c}`}>
                    {v.status === 'aprovacao' && <AlertTriangle className="w-3.5 h-3.5" />}
                    {v.status === 'aprovada' && <CheckCircle2 className="w-3.5 h-3.5" />}
                    {v.status === 'disponivel' && <HandCoins className="w-3.5 h-3.5" />}
                    {v.status === 'concluida' && <Plane className="w-3.5 h-3.5" />}
                    {b.t}
                  </span>
                </div>
