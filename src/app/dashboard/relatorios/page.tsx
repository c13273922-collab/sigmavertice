...
export default function RelatoriosPage() {
  ...
  function badgeInfo(s: string) {
    if (s === 'rascunho') return { t: 'Rascunho', c: 'bg-slate-500/15 text-slate-700' }
    if (s === 'aprovacao') return { t: 'Aprovação DG', c: 'bg-purple-500/15 text-purple-700' }
    return { t: 'Finalizado', c: 'bg-emerald-500/15 text-emerald-700' }
  }
                  ...
                  <tr key={r.protocolo} className="hover:bg-sigma-azul-50/40 transition-colors animate-fade-in-up" style={{ animationDelay: `${i * 0.05}s` }}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold ${b.c}`}>
                        {r.status === 'rascunho' && <FileBarChart className="w-3 h-3" />}
                        {r.status === 'aprovacao' && <AlertTriangle className="w-3 h-3" />}
                        {r.status === 'finalizado' && <CheckCircle2 className="w-3 h-3" />}
                        {b.t}
                      </span>
                    </td>
