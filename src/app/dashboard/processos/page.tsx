          ...
          return (
            <div key={s.label} className="rounded-xl border border-sigma-azul/10 bg-white p-4 shadow-sm animate-fade-in-up" style={{ animationDelay: `${i * 0.05}s` }}>
              <div className="flex items-center justify-between mb-2">
                <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${s.cor}`}>{s.label}</span>
                <Icon className="w-4 h-4 text-sigma-azul/50" />
              </div>
              <p className="text-2xl font-bold text-sigma-azul">{s.valor}</p>
            </div>
          )
        })}
      </div>
