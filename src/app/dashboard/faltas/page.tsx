...
export default function FaltasPage() {
  ...
  function badgeInfo(s: string) {
    if (s === 'pendente') return { t: 'Pendente', c: 'bg-red-500/15 text-red-700' }
    if (s === 'analise') return { t: 'Em análise', c: 'bg-amber-500/15 text-amber-700' }
    return { t: 'Aprovado', c: 'bg-emerald-500/15 text-emerald-700' }
  }
                  ...
                  <tr key={f.protocolo} className="hover:bg-sigma-azul-50/40 transition-colors animate-fade-in-up" style={{ animationDelay: `${i * 0.05}s` }}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold ${b.c}`}>
                        {f.status === 'pendente' && <AlertCircle className="w-3 h-3" />}
                        {f.status === 'analise' && <Clock className="w-3 h-3" />}
                        {f.status === 'aprovado' && <CheckCircle2 className="w-3 h-3" />}
                        {b.t}
                      </span>
                    </td>
