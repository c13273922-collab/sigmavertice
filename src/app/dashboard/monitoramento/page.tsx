...
export default function MonitoramentoPage() {
  ...
  function importInfo(i: string) {
    if (i === 'alta') return { t: 'Alta', c: 'bg-red-500/15 text-red-700' }
    if (i === 'media') return { t: 'Média', c: 'bg-amber-500/15 text-amber-700' }
    return { t: 'Baixa', c: 'bg-slate-500/15 text-slate-700' }
  }
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="rounded-2xl border border-sigma-azul/10 bg-white p-5 shadow-sm">
        ...
        <div className="space-y-4">
          {noticias.map((n, i) => {
            const b = importInfo(n.importancia)
            return (
              <article key={i} className="rounded-xl border border-sigma-azul/10 p-5 hover:shadow-md hover:border-sigma-dourado/30 transition-all animate-fade-in-up" style={{ animationDelay: `${i * 0.05}s` }}>
                <div className="flex flex-wrap gap-2 mb-2 text-xs items-center">
                  <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full font-bold ${b.c}`}>
                    {n.importancia === 'alta' && <AlertTriangle className="w-3.5 h-3.5" />}
                    {n.importancia === 'media' && <Bell className="w-3.5 h-3.5" />}
                    {n.importancia === 'baixa' && <Newspaper className="w-3.5 h-3.5" />}
                    Importância {b.t}
                  </span>
