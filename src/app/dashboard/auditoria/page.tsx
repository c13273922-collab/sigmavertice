...
export default function AuditoriaPage() {
  ...
  function badgeInfo(s: string) {
    if (s === 'sucesso') return { t: 'Sucesso', c: 'bg-emerald-500/15 text-emerald-700' }
    if (s === 'falha') return { t: 'Falha', c: 'bg-red-500/15 text-red-700' }
    return { t: 'Bloqueado (segurança)', c: 'bg-red-700/25 text-red-800' }
  }
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="rounded-xl border border-sigma-azul/10 bg-white p-4 shadow-sm animate-fade-in-up">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] uppercase tracking-wider text-sigma-azul/50 font-semibold">Eventos (24h)</span>
            <Database className="w-4 h-4 text-sigma-azul" />
          </div>
          <p className="text-2xl font-bold text-sigma-azul">2.841</p>
        </div>
        <div className="rounded-xl border border-sigma-azul/10 bg-white p-4 shadow-sm animate-fade-in-up" style={{ animationDelay: '0.05s' }}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] uppercase tracking-wider text-sigma-azul/50 font-semibold">Logins com sucesso</span>
            <KeyRound className="w-4 h-4 text-emerald-700" />
          </div>
          <p className="text-2xl font-bold text-emerald-700">412</p>
        </div>
        <div className="rounded-xl border border-sigma-azul/10 bg-white p-4 shadow-sm animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] uppercase tracking-wider text-sigma-azul/50 font-semibold">Falhas de autenticação</span>
            <AlertTriangle className="w-4 h-4 text-red-700" />
          </div>
          <p className="text-2xl font-bold text-red-700">37</p>
        </div>
        <div className="rounded-xl border border-sigma-azul/10 bg-white p-4 shadow-sm animate-fade-in-up" style={{ animationDelay: '0.15s' }}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] uppercase tracking-wider text-sigma-azul/50 font-semibold">Ações bloqueadas (RLS)</span>
            <ShieldAlert className="w-4 h-4 text-red-800" />
          </div>
          <p className="text-2xl font-bold text-red-800">4</p>
        </div>
        <div className="rounded-xl border border-sigma-azul/10 bg-white p-4 shadow-sm animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] uppercase tracking-wider text-sigma-azul/50 font-semibold">Arquivos verificados</span>
            <FileWarning className="w-4 h-4 text-sigma-douradoEscuro" />
          </div>
          <p className="text-2xl font-bold text-sigma-douradoEscuro">89</p>
        </div>
      </div>
                  ...
                  <tr key={l.id} className={`hover:bg-sigma-azul-50/40 transition-colors animate-fade-in-up ${l.status === 'bloqueado' ? 'bg-red-500/[0.03]' : ''}`} style={{ animationDelay: `${i * 0.04}s` }}>
                    ...
                    <td className="px-3 py-3">
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full font-bold whitespace-nowrap ${b.c}`}>
                        {l.status === 'sucesso' && <CheckCircle2 className="w-3 h-3" />}
                        {l.status === 'falha' && <AlertTriangle className="w-3 h-3" />}
                        {l.status === 'bloqueado' && <ShieldAlert className="w-3 h-3" />}
                        {b.t}
                      </span>
                    </td>
                  </tr>
